...
# 5. Start data generator (in separate terminal)
python source-db/seed_data.py

# 6. Monitor Spark logs
docker logs -f spark-processor

# 7. Query warehouse
docker exec -it postgres-warehouse psql -U warehouse -d analytics -c "SELECT * FROM orders_latest LIMIT 10;"